...
// upload routine, do not change ids in commons.html
function uploadImport(dialogId, formData) {
   formData.userCode = loginInfo.userCode;
   formData.roleCode = loginInfo.roleCode;
   $('#fileupload').fileupload({
      url: "importserver",
      autoUpload: true,
      formData: formData,
      add: function(e, data) {
         $("#filename").val(data.files[0].name);//display user choosed file
         $("#up_btn").on('click', function() {
            if (isDblClick()) return;
            confirm1('Import data', 'Import selected file into database. Are you sure?', function() {
               $("#myUploadDialog").modal('hide'); //close manually
               $(document.body).css("cursor", "progress");
               var xhr = data.submit();
            });
         });
         ...
         $(document.body).css("cursor", "default");
         console.log("----import result=" + JSON.stringify(data.result));
         proccessResult(data.result);
      },
      fail: function(e, data) {
         $(document.body).css("cursor", "default");
         console.log("----import fail=" + JSON.stringify(data));
      }
   });
   $(dialogId).modal();
}
