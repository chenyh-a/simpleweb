...
function importCustomer() {
   ...
   var formData = {
      ...
      cols: JSON.stringify(cols0)//上传文件时表单变量不支持复杂结构，转为字符串传。
   };

   uploadImport("#myUploadDialog", formData);
}
...
function proccessResult(obj) {
   ...
   if (obj.method == "sp_get_customer_by_id" && obj.tag == "QUERY_ONE") {
      ...
      refreshList("#myTable", "#checkAll");
      console.log("---import result obj=" + obj);
      ...
      callajax("queryserver", obj);//get one record after get select values
...
function listCustomer() {
   ...
   showList("#myTable", "queryserver", opt, columns, columnsDefs, queryData);
}
...
function exportCustomer() {
   ...
   obj.watermark = true;// print userCode as watermark
   obj.cols = cols;
   obj.data = {};
